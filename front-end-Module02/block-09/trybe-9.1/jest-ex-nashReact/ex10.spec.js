...
const lookup = (login, property) => {
  ...
  if (users.find(name => name.login === login) === undefined) {
    throw new Error(/Could not find user/);
  }
  const user = users.find(person => person.login === login);
  
  if (Object.keys(user).find(prop => prop === property) === undefined) {
    throw new Error(/Could not find property/);
  }
  
  if (property === 'likes') return user.likes;
  if (property === 'lastName') return user.lastName;  
  // END
};
