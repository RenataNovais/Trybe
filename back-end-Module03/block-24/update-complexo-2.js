// Você continuará utilizando o mesmo dataset de filmes do dia anterior. 
// Por isso, hora de voltar a coleção movies para o seu estado original:
...
db.movies.find({ratings: {$gt: 103}, category: {$all: ["adventure"]}}, {title: 1, ratings: 1, category: 1, "_id": 0});
...
db.movies.find({$or: [{ratings: {$gt: 199}}, {category: {$all: ["sci-fi"]}}]}, {title: 1, ratings: 1, category: 1, "_id": 0});

// 14. Retorne os filmes em que o ratings possua tamanho 4 ou que seja da category "adventure" ou "family", mas que não tenha o imdbRating menor que 7.
db.movies.find({$and: [{$or: [{ratings: {$size: 4}}, {category: {$all: ["adventure", "family"]}}]}, {imdbRating: {$not: {$lt: 7}}}]});
