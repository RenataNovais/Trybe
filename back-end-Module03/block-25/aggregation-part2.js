...
// Algumas dicas: arredonde para baixo o valor da idade;
// calcule a idade usando a diferença entre a data corrente e a data de nascimento;
// 1 dia é igual a 86400000 milissegundos.
db.clientes.aggregate([ // gabarito
  {
    $addFields: {
      idade: {
        $floor:{
...
// Exercício 3: Remova os estágios $count e $match do exercício anterior e adicione um estágio no pipeline que coloque as
// compras do cliente no campo compras.
db.clientes.aggregate([
  {
    $addFields: {
      idade: {
...
db.clientes.aggregate([
  ...
  {$match: {"comprasCliente.dataVenda": {
...
db.clientes.aggregate([
  ...
  {$addFields: {totalCompras: {$size: "$comprasCliente"}}},
  {$sort: {totalCompras: -1}},
  { $limit: 10 }
]);

// Exercício 7: Para esses clientes, adicione um campo chamado compras.valorComDesconto em todas as compras do período,
...
db.clientes.aggregate([
  ...
  {
    $addFields: {
      "comprasCliente.valorComDesconto": {
        $subtract: [
          "$comprasCliente.valorTotal",
          { $multiply: ["$comprasCliente.valorTotal", 0.10] }
        ]
      }
    }
  }
